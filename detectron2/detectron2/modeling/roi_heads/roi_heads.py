...
@ROI_HEADS_REGISTRY.register()
class StandardROIHeadswGraph(StandardROIHeads):
    ...
    def forward(
        ...
        proposals: List[Instances],
        targets: Optional[List[Instances]] = None,
    ) -> Tuple[List[Instances], Dict[str, torch.Tensor], torch.Tensor]:
        """
        See :class:`ROIHeads.forward`.
        Returns :
        ...
        if self.training:
            losses, box_features, proposals = self._forward_box(features, proposals)
            # Usually the original proposals used by the box head are used by the mask, keypoint
            # heads. But when `self.train_on_pred_boxes is True`, proposals will contain boxes
            # predicted by the box head.
            losses.update(self._forward_mask(features, proposals))
        ...
        if self.training:
            ...
            if self.train_on_pred_boxes:
                with torch.no_grad():
                    ...
                    for proposals_per_image, pred_boxes_per_image in zip(proposals, pred_boxes):
                        proposals_per_image.proposal_boxes = Boxes(pred_boxes_per_image)
            return losses, box_features, proposals
        else:
            pred_instances, _ = self.box_predictor.inference(predictions, proposals)
            return pred_instances, box_features
