...
from detectron2.modeling.graph.graph_connection import build_graph_connection
...
__all__ = ["PanopticFPN","PanopticFPNwGraph"]
...
@META_ARCH_REGISTRY.register()
class PanopticFPNwGraph(PanopticFPN):
    """
    Implement the paper with Graph connection.
    """
    ...
    def __init__(
        self,
        *,
        # New modules: graph connection, including  box head, mask head and segment branch
        graph_connection: nn.Module,
        roi_head_name: str,
        seg_branch_name: str,
        **kwargs
    ):
        ...
        """
        super().__init__(**kwargs)
        assert "Graph" in roi_head_name
        assert "Graph" in seg_branch_name
    ...
    def from_config(cls, cfg):
        ret = super().from_config(cfg)
        ret["graph_connection"] = build_graph_connection(cfg, ret["backbone"].output_shape())
        ret["roi_head_name"] = cfg.MODEL.ROI_HEADS.NAME
        ret["seg_branch_name"] = cfg.MODEL.SEM_SEG_HEAD.NAME
        return ret
    ...
    def inference(
        ...
        images = self.preprocess_image(batched_inputs)
        features = self.backbone(images.tensor)
        sem_seg_results, _, feat, seg_score = self.sem_seg_head(features, None)
        proposals, _ = self.proposal_generator(images, features, None)
        detector_results, _, box_features = self.roi_heads(images, features, proposals, None)


        #############################
        # Graph op
        #############################
        detector_results, sem_seg_results, _ = self.graph_connection(
            box_features, proposals, features,
            feat, seg_score, None,
        )
...
def combine_semantic_and_instance_outputs(
    ...
    sorted_inds = sorted_instances(instance_results, panoptic_seg)
...
def sorted_instances(ins, panoptic_seg):
    # first build a list
    id_ins = [(i, ins[i].scores.item(), ins[i].pred_masks.to(dtype=torch.bool, device=panoptic_seg.device).sum().item()) for i in range(len(ins))]
    sorted_id_ins = sorted(id_ins, key=cmp_to_key(mycmp_v2))
    sorted_ids = [x[0] for x in sorted_id_ins]
    return torch.LongTensor(sorted_ids).to(device=panoptic_seg.device)

def mycmp_v2(a, b):
    if a[1] > 0.725 and b[1] > 0.725:
        area_a = a[2]
        area_b = b[2]
        # area_a = COCOmask.area(a['segmentation'])
        # area_b = COCOmask.area(b['segmentation'])
        if area_a < area_b:
            return -1
        if area_a > area_b:
            return 1
